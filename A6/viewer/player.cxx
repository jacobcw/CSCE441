...
//Interpolate motion
vector CatmullRom(float f,vector p0,vector p1,vector p2, vector p3,float n)
{
	float tau = 0.50;
	f = (f/n);
	...
	float f3 = f*f*f;
	float f2 = f*f;
	vector a = (p0*(-tau))+(p1*(2-tau))+(p2*(tau-2))+(p3*(tau));
	vector b = (p0*(2*tau))+(p1*(tau-3))+(p2*(3-2*tau))+(p3*(-tau));
	vector c = (p0*(-tau))+(p2*(tau));
	vector d = p1;
	
	vector interp = (a*f3)+(b*f2)+(c*f)+d;
...
{
	if(size<4) {printf("Not enough keyframes for interpolation");}
	if(interpolated==true) {printf("Already Interpolated");}
	else{
		int total = pSampledMotion->m_NumFrames;
		iMotion = new Motion(total);
		iMotion->SetPosturesToDefault();
		int frame = 1;
		vector p0,p1,p2,p3; //points
		vector r0,r1,r2,r3; //rotations

		vector t0,t1,t2,t3; //translation

		curr = 0;
		if(keyframes[size-1]==total) size--;
		int current = keyframes[curr];
		int first = keyframes[0];
		int last = keyframes[size-1];
		int next = keyframes[curr+1];
		int prev = NULL;
		int nextNext = keyframes[curr+2];
		
		Posture* fPost = pSampledMotion->GetPosture(0);
		Posture* lPost = pSampledMotion->GetPosture(total-1);
		
		iMotion->SetPosture(0,*fPost);

		for(frame;frame<total-1;frame++)
		{
			if(frame==next+1){
				prev=current;
				curr++; 
			...
			Posture* iPosture = new Posture;
			if(frame<=first){ // first frames
				p0=pSampledMotion->GetPosture(0)->root_pos; 
				p1=pSampledMotion->GetPosture(0)->root_pos;
				p2=pSampledMotion->GetPosture(current)->root_pos;
				p3=pSampledMotion->GetPosture(next)->root_pos;
				iPosture->root_pos=CatmullRom((float)frame,p0,p1,p2,p3,(float)first);
				for(int b=0; b<=NUM_BONES_IN_ASF_FILE; b++){
					r0=pSampledMotion->GetPosture(0)->bone_rotation[b]; 
					r1=pSampledMotion->GetPosture(0)->bone_rotation[b];
					r2=pSampledMotion->GetPosture(current)->bone_rotation[b];
					r3=pSampledMotion->GetPosture(next)->bone_rotation[b];
					iPosture->bone_rotation[b]=CatmullRom((float)frame,r0,r1,r2,r3,(float)first);
					t0=pSampledMotion->GetPosture(0)->bone_translation[b];
					t1=pSampledMotion->GetPosture(0)->bone_translation[b];
					t2=pSampledMotion->GetPosture(current)->bone_translation[b];
					t3=pSampledMotion->GetPosture(next)->bone_translation[b];
					iPosture->bone_translation[b]=CatmullRom((float)frame,t0,t1,t2,t3,(float)first);
				}
			}
			else if(frame>last){ // last frames
				p0=pSampledMotion->GetPosture(prev)->root_pos; 
				p1=pSampledMotion->GetPosture(current)->root_pos;
				p2=pSampledMotion->GetPosture(next)->root_pos;
				p3=pSampledMotion->GetPosture(next)->root_pos;
				iPosture->root_pos=CatmullRom((float)(frame-last),p0,p1,p2,p3,(float)(total-last));
				for(int b=0; b<=NUM_BONES_IN_ASF_FILE; b++){
					r0=pSampledMotion->GetPosture(prev)->bone_rotation[b]; 
					r1=pSampledMotion->GetPosture(current)->bone_rotation[b];
					r2=pSampledMotion->GetPosture(next)->bone_rotation[b];
					r3=pSampledMotion->GetPosture(next)->bone_rotation[b];
					iPosture->bone_rotation[b]=CatmullRom((float)(frame-last),r0,r1,r2,r3,(float)(total-last));
					t0=pSampledMotion->GetPosture(prev)->bone_translation[b];
					t1=pSampledMotion->GetPosture(current)->bone_translation[b];
					t2=pSampledMotion->GetPosture(next)->bone_translation[b];
					t3=pSampledMotion->GetPosture(next)->bone_translation[b];
					iPosture->bone_translation[b]=CatmullRom((float)(frame-last),t0,t1,t2,t3,(float)(total-last));
				}
			}
			else if(frame<=keyframes[1]){ // first frames
				p0=pSampledMotion->GetPosture(0)->root_pos; 
				p1=pSampledMotion->GetPosture(first)->root_pos;
				p2=pSampledMotion->GetPosture(keyframes[1])->root_pos;
				p3=pSampledMotion->GetPosture(keyframes[2])->root_pos;
				iPosture->root_pos=CatmullRom((float)(frame-first),p0,p1,p2,p3,(float)(keyframes[1]-first));
				for(int b=0; b<=NUM_BONES_IN_ASF_FILE; b++){
					r0=pSampledMotion->GetPosture(0)->bone_rotation[b]; 
					r1=pSampledMotion->GetPosture(first)->bone_rotation[b];
					r2=pSampledMotion->GetPosture(keyframes[1])->bone_rotation[b];
					r3=pSampledMotion->GetPosture(keyframes[2])->bone_rotation[b];
					iPosture->bone_rotation[b]=CatmullRom((float)(frame-first),r0,r1,r2,r3,(float)(keyframes[1]-first));
					t0=pSampledMotion->GetPosture(0)->bone_translation[b];
					t1=pSampledMotion->GetPosture(first)->bone_translation[b];
					t2=pSampledMotion->GetPosture(keyframes[1])->bone_translation[b];
					t3=pSampledMotion->GetPosture(keyframes[2])->bone_translation[b];
					iPosture->bone_translation[b]=CatmullRom((float)(frame-first),t0,t1,t2,t3,(float)(keyframes[1]-first));
				}
			}
				...
				p1=pSampledMotion->GetPosture(current)->root_pos;
				p2=pSampledMotion->GetPosture(next)->root_pos;
				p3=pSampledMotion->GetPosture(nextNext)->root_pos;
				iPosture->root_pos=CatmullRom((float)(frame-current),p0,p1,p2,p3,(float)(next-current));
				for(int b=0; b<=NUM_BONES_IN_ASF_FILE; b++){
					r0=pSampledMotion->GetPosture(prev)->bone_rotation[b];
					r1=pSampledMotion->GetPosture(current)->bone_rotation[b];
					r2=pSampledMotion->GetPosture(next)->bone_rotation[b];
					r3=pSampledMotion->GetPosture(nextNext)->bone_rotation[b];
					iPosture->bone_rotation[b] = CatmullRom((float)(frame-current),r0,r1,r2,r3,(float)(next-current));
					t0=pSampledMotion->GetPosture(prev)->bone_translation[b];
					t1=pSampledMotion->GetPosture(current)->bone_translation[b];
					t2=pSampledMotion->GetPosture(next)->bone_translation[b];
					t3=pSampledMotion->GetPosture(nextNext)->bone_translation[b];
					iPosture->bone_translation[b]=CatmullRom((float)(frame-current),t0,t1,t2,t3,(float)(next-current));
				}
			}
			iMotion->SetPosture(frame,*iPosture);
		}

		iMotion->SetPosture(total-1,*lPost);
		interpolated=true;
		displayer.loadMotion(iMotion);
		glwindow->redraw();
		Rewind=ON;	Play=OFF;
	}
...
void view_callback(Fl_Button *button, void *) 
{
	if(size==0) cout<< "Save Keyframes First!"<< endl;
	else{
		nFrameNum = keyframes[curr];
		for (int i = 0; i < displayer.numActors; i++)
			displayer.m_pActor[i]->setPosture(displayer.m_pMotion[i]->m_pPostures[displayer.m_pMotion[i]->GetPostureNum(keyframes[curr])]);
		Fl::flush();
		glwindow->redraw();
		Play=OFF;
		printf("%d/%d - %d\n",curr+1,size,keyframes[curr]);
		if(curr+1==size) {curr=0;}
		else ++curr;
	}
}
